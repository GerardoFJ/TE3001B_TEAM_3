...
Simulación 3-DOF + Computed Torque + Graficación en Tiempo Real
...
# ─────────────────────────────────────────────
L1, L2, L3 = 0.35, 0.30, 0.20      # longitudes de eslabones [m]
...
def fk_3r(q):
    ...
    Returns:
        np.ndarray: (x, y) posición del efector final en metros.
    """
    q1, q2, q3 = q
    x1 = L1 * np.cos(q1)
    y1 = L1 * np.sin(q1)
...
def fk_3r_full(q):
    ...
    Returns:
        np.ndarray: Matriz (4×2) con posiciones [base, j1, j2, EF].
    """
    q1, q2, q3 = q
    p0 = np.array([0.0, 0.0])
...
# ─────────────────────────────────────────────
#  JACOBIANO ANALÍTICO 3R PLANAR  J ∈ ℝ^{2×3}
# ─────────────────────────────────────────────
def jacobian_3r(q):
    """
...
def coriolis_matrix(q, dq):
    ...
    Calculada vía diferenciación numérica de M(q) (método Christoffel).
...
def computed_torque(q, dq, q_des, dq_des, ddq_des,
    ...
    Ley de control de Computed Torque (cancelación exacta de no-linealidades).
        ...
        Control: τ = M(q)·aᵈ + C(q,q̇)·q̇ + g(q)
        donde:   aᵈ = q̈ᵈ + Kv(q̇ᵈ - q̇) + Kp(qᵈ - q)
...
# ─────────────────────────────────────────────
#  COMUNICACIÓN DE RED MAESTRO (CLIENTE UDP)
# ─────────────────────────────────────────────
...
class MasterRobot:
    ...
    def __init__(self, slave_ip="127.0.0.1"):
        ...
        self.v_step = 0.25   # [m/s] por pulsación de tecla
    ...
    def step(self):
        ...
        tau, e, de = computed_torque(
            ...
            F_ext=0.3 * self.net.Fe   # factor de feedback háptico β=0.3
        )
...
def setup_plots(robot):
    ...
    ax_robot.plot(hole_x, hole_y, 'x', color='white',
    ...
    ax_force.axhline(y=2, color='#FF4444', linewidth=1.0,
                     linestyle='--', alpha=0.6, label='Umbral 2N')
...
def main(slave_ip):
    ...
    def on_key_press(event):
        ...
        if   event.key == 'w':      robot.v_cart = np.array([0,  v])
            ...
            np.savez('master_data.npz',
                     t=robot.hist_t,
                     q=robot.hist_q,
                     tau=robot.hist_tau,
                     Fe=robot.hist_Fe,
                     x=robot.hist_x)
            print("Datos guardados en master_data.npz")
            plt.close()
    ...
    print("=" * 60)
    print("  TE3001B — Robot MAESTRO iniciado")
    print(f"  Conectando al esclavo en: {slave_ip}")
    print("  Controles: W/A/S/D = mover EF | Q/E = pinza | ESC = salir")
    print("=" * 60)
    plt.show()
