...
HOLE_RADIUS   = 0.009                 # radio del agujero (tolerancia 1 mm)
...
def impedance_control(q, dq, x_des, dx_des, q_des_prev,
    ...
    x_cur  = fk_3r(q)
    J      = jacobian_3r(q)
    dx_cur = J @ dq          # velocidad cartesiana actual
    ...
    # Compensación de gravedad y Coriolis (siempre necesaria)
    g_vec = gravity_vector(q)
    C_mat = coriolis_matrix(q, dq)
...
class SlaveNetServer:
    ...
    def __init__(self, master_ip="127.0.0.1", port_rx=9001, port_tx=9002):
        ...
        self.gripper = True
        self.master_addr = (master_ip, port_tx)
    ...
    def send_force(self, Fe, contact):
        ...
        try:
            self.sock.sendto(msg.encode(), (self.master_ip, self.master_port))
        except Exception:
            pass
...
class SlaveRobot:
    ...
    def step(self):
        ...
        # Velocidad cartesiana deseada (usamos cero para simplicidad)
        dx_des = np.zeros(2)
        ...
        # Guardar q_des para próximo paso
        self.q_des_prev = self.ik_dls(x_des)
...
def setup_slave_plots(robot):
    ...
    hole_open = plt.Rectangle(
        (HOLE_CENTER[0]-HOLE_RADIUS, HOLE_CENTER[1]-0.005),
        2*HOLE_RADIUS, 0.025,
        color='#0d1117', zorder=2)
    ax_robot.add_patch(hole_open)
    ...
    peg_line,  = ax_robot.plot([], [], '-',  color=C[3], linewidth=5,
                               zorder=4, solid_capstyle='round')
    state_text  = ax_robot.text(0.02, 0.96, '',
                                transform=ax_robot.transAxes,
                                color='#FFD700', fontsize=10,
                                fontweight='bold',
                                verticalalignment='top')

    # Flecha de fuerza de contacto (inicializada sin visibilidad)
    force_arrow = ax_robot.annotate('',
                                    xy=(0, 0), xytext=(0, 0),
    ...
    line_Fx, = ax_force.plot([], [], color=C[4], linewidth=2.0,
                             label='Fx contacto')
    line_Fy, = ax_force.plot([], [], color=C[5], linewidth=2.0,
                             label='Fy contacto')
...
def main(master_ip):
    ...
    (fig, axes,
     robot_artists, force_lines,
     lines_tau, err_lines) = setup_slave_plots(robot)
    ...
    def animate(frame):
        ...
        # ── Flecha de fuerza de contacto ──────────────────
        Fc_cur = robot.hist_Fc[robot.idx % 500]
        Fc_mag = np.linalg.norm(Fc_cur)
        if Fc_mag > 0.5:
            scale = 0.05 / max(Fc_mag, 1.0)
            force_arrow.set_position(ef + Fc_cur * scale)
            force_arrow.xy = ef
        else:
            force_arrow.set_position(ef)
            force_arrow.xy = ef
        ...
        ax_force.relim()
        ax_force.autoscale_view(scalex=False)
        ...
        ax_tau.relim()
        ax_tau.autoscale_view(scalex=False)
        ...
        return ([link_line, peg_line, state_text]
                + lines_tau + [line_Fx, line_Fy, line_ex, line_ey])

    def on_key_press(event):
        if event.key == 'escape':
            running[0] = False
            np.savez('slave_data.npz',
                     t=robot.hist_t,
                     tau=robot.hist_tau,
                     Fc=robot.hist_Fc,
                     x=robot.hist_x,
                     ex=robot.hist_ex)
            print("Datos guardados en slave_data.npz")
            plt.close()
    ...
    print("=" * 60)
    print("  TE3001B — Robot ESCLAVO iniciado")
    print(f"  IP Maestro: {master_ip}")
    print("  Esperando comandos en UDP:9001 ...")
    print("  ESC → salir y guardar datos")
    print("=" * 60)
    plt.show()
