...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\columnwidth]{figures/fig_A2_latency_histogram.pdf}
  \caption{Histogramas de intervalo entre llegadas de mensajes para los cuatro
    tópicos principales. \texttt{/master/joint\_states}: $\mu=7.12$~ms,
    $\sigma=7.18$~ms, P99$=42.70$~ms. \texttt{/slave/joint\_states}: $\mu=7.05$~ms,
    P99$=39.66$~ms. \texttt{/teleop/operator\_force\_direct}: $\mu=21.50$~ms,
    P99$=63.19$~ms. \texttt{/master/servo/twist\_cmds}: $\mu=20.52$~ms,
    P99$=59.73$~ms.}
  \label{fig:latency}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\columnwidth]{figures/fig_F1_operator_force.pdf}
  \caption{Señales del sensor físico ESP32 maestro (tópico
    \texttt{/teleop/operator\_force\_direct}) durante el ensayo de 37.6~s.
    Arriba: fuerzas $F_x$ y $F_y$ del joystick analógico ($\pm3$~N máx.).
    Abajo: fuerza $F_z$ de compresión del resorte con sensor ultrasónico
    HC-SR04 (máximo registrado: 38.5~N). Las zonas grises indican los
    7 eventos de fuerza activa ($\|\bm{F}_{op}\|>2$~N).}
  \label{fig:op_force}
\end{figure}
...
\begin{enumerate}[label=(\alph*)]
  ...
  \begin{tabular}{lll}
    ...
    Python   & 3.10 & 3.10 \\
    xArm SDK & xarm\_ros2 (Humble) & xarm\_ros2 (Humble) \\
    \bottomrule
  \end{tabular}}
  \smallskip
...
3 ensayos de 30--40~s cada uno, con criterio de éxito:
...
\begin{enumerate}[label=(\alph*)]
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    Joint 1 & 0.793 & 2.934 & $\approx7$ \\
    Joint 2 & 1.040 & 5.433 & $\approx7$ \\
    Joint 3 & 0.314 & 1.131 & $\approx7$ \\
    Joint 4 & 0.219 & 0.560 & $\approx7$ \\
    Joint 5 & 1.162 & 3.291 & $\approx7$ \\
    Joint 6 & 0.685 & 1.780 & $\approx7$ \\
    \midrule
    \textbf{Promedio} & \textbf{0.702} & \textbf{2.522} & $\approx7$ \\
    \bottomrule
  \end{tabular}}
  \smallskip
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\columnwidth]{figures/fig_E2_joint_tracking_error.pdf}
  \caption{Error de seguimiento articular $e_i(t) = q_{m,i}(t) - q_{s,i}(t)$
    para las 6 articulaciones durante el ensayo de 37.6~s. Los valores RMS
    obtenidos son: J1$=0.793°$, J2$=1.040°$, J3$=0.314°$, J4$=0.219°$,
    J5$=1.162°$, J6$=0.685°$ (media$=0.702°$). Las zonas grises indican
    los 7 eventos de contacto detectados ($\|\bm{F}_{op}\|>2$~N).}
  \label{fig:joint_error}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\columnwidth]{figures/fig_E2b_joint_positions.pdf}
  \caption{Posiciones articulares del maestro (línea sólida) y esclavo
    (línea discontinua) superpuestas para las 6 articulaciones. Se aprecia
    el retardo de seguimiento y los instantes de mayor divergencia
    durante cambios de dirección rápidos (joint 2 y joint 5).}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\columnwidth]{figures/fig_admittance_velocity.pdf}
  \caption{Comandos de velocidad cartesiana TwistStamped publicados a MoveIt~Servo
    a 50~Hz para el maestro (arriba) y el esclavo (abajo). La línea punteada
    indica el límite $v_{max}=\pm100$~mm/s. Se observan saturaciones durante
    los eventos de fuerza más intensos (evento~7, t$>19.5$~s).}
  \label{fig:admittance_velocity}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\columnwidth]{figures/fig_EE_pose.pdf}
  \caption{Trayectoria del efector final del maestro (/teleop/master\_pose)
    y velocidad cartesiana publicada por el nodo de admitancia
    (/teleop/master\_velocity). Las posiciones $p_x$, $p_y$, $p_z$
    muestran el movimiento tridimensional durante los 37.6~s del ensayo.}
  \label{fig:ee_pose}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\columnwidth]{figures/fig_E3a_admittance_response.pdf}
  \caption{Respuesta dinámica del sistema durante los eventos de fuerza.
    Panel superior: fuerza del operador $\|\bm{F}_{op}(t)\|$ con umbral
    $F_{th}=2$~N. Panel central: velocidad cartesiana de salida del nodo de
    admitancia (maestro). Panel inferior: error de seguimiento articular
    normalizado. Las zonas grises indican los 7 eventos activos detectados.}
  \label{fig:admittance_response}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\columnwidth]{figures/fig_E3b_admittance_validation.pdf}
  \caption{Validación del modelo de admitancia: diagrama de dispersión
    $\|\bm{F}_{op}\|$ vs. $\|\bm{v}_{cmd}\|$ obtenido del sensor ESP32 maestro
    y los comandos TwistStamped publicados a MoveIt~Servo. La pendiente de la
    recta de regresión confirma la proporción fuerza-velocidad definida por
    los parámetros $M_d=2$~kg y $B_d=10$~N·s/m.}
  \label{fig:admittance_validation}
\end{figure}

La fuerza máxima registrada por el sensor ESP32 maestro durante el ensayo fue
$F_{z,max}=38.5$~N (compresión del resorte, $k=1000$~N/m). Las fuerzas de joystick
se mantuvieron en $F_{x,y} \in [-3.0, 3.0]$~N, consistente con
\texttt{JOY\_MAX\_FORCE\_N}$=3.0$~N configurado en \texttt{config.h}. El nivel de ruido
en reposo (sensor ultrasónico, zona muerta $\pm 0.3$~cm) produjo $F_z \approx 0$~N
cuando no hay compresión activa ($|F_z| < F_{z,min}=0.3$~N).
La proporcionalidad $F_{haptic} \propto \alpha \hat{F}_{slave}$ con $\alpha=0.3$
se verificó mediante los comandos de velocidad publicados: al aumentar
$\|\bm{F}_{op}\|$ el sistema de admitancia genera velocidades proporcionales
clippeadas a $v_{max}=0.10$~m/s (ver Fig.~\ref{fig:admittance_validation}).
...
El ensayo en Isaac~Sim mostró estabilidad robusta durante el evento~7
(t$=19.5$--$37.6$~s, 18.1~s de contacto continuo) con $F_z$ llegando a
38.5~N sin oscilaciones sostenidas. El valor teórico de $\alpha_{crit}=2B_d/K_{env}$
con $B_d=10$~N·s/m y $K_{env}\approx 1000$~N/m (rigidez simulada) da
$\alpha_{crit}\approx 0.020$; el valor operativo $\alpha=0.3$ permanece estable
gracias al clippeado de velocidad a $v_{max}=0.10$~m/s que limita la energía
inyectada al loop. No se observó acoplamiento de inestabilidad simultáneo en
la configuración Isaac~Sim (esclavo simulado) + ESP32 (maestro físico) durante
...
de ${\approx}1.5$~N, lo que es competitivo para aplicaciones de
...
${\approx}1.5$~N frente a referencia con báscula. Dos sensores ESP32
